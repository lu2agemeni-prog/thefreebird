...
// ============================================================================
// PatientAppointments — لوحة المريض للمواعيد.
// الإصلاحات: معالجة أخطاء التحميل مع إعادة محاولة (كانت فشلًا صامتًا)،
// أخطاء القاعدة الودّية (تعارض المواعيد/أيام العمل) بدل alert عام،
// عرض أيام عمل الطبيب من عمود working_days، وحالات من lib/types.
// التحقق من التعارض نفسه يتم في القاعدة (SECURITY DEFINER trigger)
...
export function PatientAppointments() {
  ...
  const fetchClinicsAndDoctors = async () => {
    ...
    if (clinicsErr) {
      ...
      if (clinics.length === 0) setClinics([]);
    } else if (clinicsData) setClinics(clinicsData);

    const { data: doctorsData } = await supabase
      .from('doctors')
      .select('profile_id, clinic_id, specialty, working_days, profiles(first_name, last_name)');
    if (doctorsData) setDoctors(doctorsData);
  };

  const filteredDoctors = doctors.filter((d) => !selectedClinic || d.clinic_id === selectedClinic);
  const currentDoctor = doctors.find((d) => d.profile_id === selectedDoctor);

  const handleBook = async (e: React.FormEvent) => {
    e.preventDefault();
    setBookingError(null);
    if (!selectedClinic || !appointmentDate || !appointmentTime) {
      setBookingError('الرجاء استكمال جميع البيانات الأساسية للحجز.');
      return;
    }
        ...
        <Card className="border-emerald-100 shadow-md">
          ...
          <CardContent className="pt-6">
            {clinics.length === 0 ? (
              <ErrorState
                compact
                message="تعذر تحميل العيادات والأطباء حاليًا."
                onRetry={fetchClinicsAndDoctors}
              />
            ) : (
              <form onSubmit={handleBook} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      <option value="">
                        {filteredDoctors.length === 0 ? 'لا يوجد أطباء لهذه العيادة' : '-- أي طبيب --'}
                      </option>
                    ...
                    {currentDoctor?.working_days && (
                      <p className="text-xs text-gray-500 mt-1">
                        أيام استقبال الطبيب: {workingDaysLabel(currentDoctor.working_days)}
                      </p>
                    )}
                  </div>
                  <div>
      ...
      <Card>
        <CardContent className="p-0">
          {loading ? (
            ...
            <ErrorState message="تعذر تحميل مواعيدك." onRetry={fetchAppointments} />
