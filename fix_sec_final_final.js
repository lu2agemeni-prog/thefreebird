...
let parts = c.split('  const handleCallNext = async () => {');
if (parts.length >= 2) {
...
`;
  let after = parts.slice(1).join('  const handleCallNext = async () => {');
  fs.writeFileSync('components/dashboards/secretary/SecretaryCallQueue.tsx', newTop + "  const handleCallNext = async () => {" + after);
  console.log("Fixed SecretaryCallQueue.tsx!");
} else {
  console.log("Could not find handleCallNext in SecretaryCallQueue.tsx");
}
